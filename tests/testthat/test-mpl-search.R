...
test_that("tree search finds shortest tree", {
  ...
  expect_equal(InapplicableFitch(BasicSearch(start_tree, dataset, Rearrange=NNICore, verbosity=0), dataset),
               InapplicableFitch(true_tree, dataset), 3)
  expect_equal(InapplicableFitch(BasicSearch(start_tree, dataset, Rearrange=SPRCore, verbosity=0), dataset),
               InapplicableFitch(true_tree, dataset), 3)
  expect_equal(InapplicableFitch(BasicSearch(start_tree, dataset, Rearrange=TBRCore, verbosity=0), dataset),
               InapplicableFitch(true_tree, dataset), 3)
