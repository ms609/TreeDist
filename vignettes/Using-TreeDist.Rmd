...
This convenience function returns the variation of clustering information
between two trees, normalized against the total information content of all
partitions.  For more on different tree distance metrics, see the
[tree distance vignette](./tree-distances.html). #TODO check link

## Visualizing the matching

Metrics such as the variation of clustering information rely on matching
each split within a tree with another split in the other tree.  We can view an
optimal matching:
...
```{r}
splits <- IdentifySplits(Tree2Splits(tree2))
splits
```
