...
The Robinson-Fould, Nye _et al_. and Quartet Metrics (&#9661;) are normalized 
...
strong correlation with the Nye _et al_. and Quartet metrics.  The 
...
metric, notwithstanding a weak correlation with the Nye _et al_. measure.
...
tree space for all `r NRooted(6)` rooted six-tip trees.  A rooted tree is equivalent to 
...
of @Kendall2016 on all `r NUnrooted(7)` unrooted seven-tip trees.
...
trees.  Note that the Nye _et al_. and Quartet metrics are pectinate, in contrast
...
pair of trees.  Rows and columns correspond to each of the `r NUnrooted(7)` 
...
```{r 7-tip-trees-2, echo=FALSE, message=FALSE, cache=TRUE, fig.asp=1/2, fig.width=7, out.width='90%', fig.align='center'}
par(mfrow=c(2, 4), mar=rep(0.8, 4))
...
is largely independent of tree topology.  The Nye _et al_. and quartet metrics
