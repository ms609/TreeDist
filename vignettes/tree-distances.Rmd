...
for (method in names(abbrevs)) AddLine(method, method %in% c('msd', 'path'))
...
toCorrelate <- c('vai', 'vpi', 'vci', 'nts', 'msd', 'qd', 'path')
lapply(seq_along(toCorrelate), function(i)
  lapply(seq_along(toCorrelate), function(j) {
    if (i == j) {
      plot.new()
      text(0.5, 0.5, abbrevs[i], col=methodCol[i])
      box(which='plot', col=methodCol[i], lwd=3)
    } else if (i < j) {
