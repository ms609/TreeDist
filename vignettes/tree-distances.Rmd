...
methodCol <- cbPalette8[c(1:5, 6, 7, 8, 8)]
...
AddLine('msd', function (x) x / rep((nLeaves * nLeaves * 3 / 8), each=2))
AddLine('path', function (x) x / rep((nLeaves * 8), each=2))
AddLine('spr')
AddLine('rf')
...
PlotMDS <- function(method) {
  space <- MASS::isoMDS(td7[[distances]], k=2, trace=FALSE)
  pts <- space$points
  ...
  legend('top', legend=abbrevs[method], bty='n', text.font=2)
}
