...
Note that the erratic shape of the path and Matching Splits distances likely 
reflects the difficulty of calculating a normalizing constant for an n-tip tree.
...
TableImage <- function (method) {
  ...
  legend('topright', legend=abbrevs[method], bty='n')
}
```
...
the trees being compared, as depicted below.
...
```{r 7-tip-trees-3, echo=FALSE, message=FALSE, cache=TRUE, fig.asp=1/2, fig.width=7, out.width='90%', fig.align='center'}
par(mfrow=c(2, 4), mar=rep(0.8, 4))
...
Trees are prominently non-uniformly distributed in SPR and RF tree space; moreover,
trees are grouped by shape
