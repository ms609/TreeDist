...
This document gives an overview of topological distance measures, and examples
...
(Instructions for loading phylogenetic trees into R can be found [elsewhere](https://ms609.github.io/TreeTools/articles/load-trees.html).)
...
TreeDist contains an inefficient implementation that allows matched splits to be
easily visualized:
...
phangorn provides a less efficient implementation that throws an error message
when trees are not binary:
```{r}
phangorn::RF.dist(tree1, tree2, normalize = FALSE)
```

To suppress the warning message, and to normalize the output against the total 
number of splits present, use:

```{r}
suppressMessages(phangorn::RF.dist(tree1, tree2, normalize = TRUE))
```

`tree1` and `tree2` can be multiPhylo objects containing multiple trees, in
which case each tree in `tree1` will be compared with each tree in `tree2`.

# Generalized Robinson-Foulds distances

TreeDist contains functions for various Generalized Robinson-Foulds 
...
The path distance can be calculated using:
