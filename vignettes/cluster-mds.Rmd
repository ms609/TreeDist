...
par(mar = rep(1.5, 4))
silmds <- ClusterMDS(clust, distances, cmds, trace = TRUE, stability = 12)

layout(LayoutRow(1, 1))
par(mar = rep(1.5, 4))
plot(points, col = clust, cex = 1.5, asp = 1, axes = FALSE, ann = FALSE)
hist(distances)

layout(LayoutRow(1, 1))
par(mar = rep(1.5, 4))
plot(silmds, col = clust, cex = 1.5, asp = 1, axes = FALSE, ann = FALSE)
hist(dist(silmds), main = "SilMDS")

layout(LayoutRow(1, 1))
par(mar = rep(1.5, 4))
plot(cmds, col = clust, cex = 1.5, asp = 1, axes = FALSE, ann = FALSE)
hist(dist(cmds), main = "Classic MDS")
```

Two gaussian clusters with same mean & variance

```{r 2-clust, message = FALSE}
nCl <- 2
clust <- rep(seq_len(nCl), c(32, 32))
nPts <- length(clust)
points <- matrix(rnorm(nPts * hiDim, 1), nPts, hiDim)
distances <- dist(points)
cmds <- cmdscale(distances)

silmds <- ClusterMDS(clust, distances, cmds, trace = TRUE, stability = 22)

layout(LayoutRow(2, 1))
par(mar = rep(1.5, 4))
plot(points, col = clust, cex = 1.5, asp = 1, axes = FALSE, ann = FALSE)
...
```
Two Gaussian clusters with same mean, different variance

```{r 2-clust-smdv, message = FALSE}
nCl <- 2
clust <- rep(seq_len(nCl), c(32, 32))
nPts <- length(clust)
points <- matrix(rnorm(nPts * hiDim, 1), nPts, hiDim) * clust
distances <- dist(points)
cmds <- cmdscale(distances)

silmds <- ClusterMDS(clust, distances, cmds, trace = TRUE, stability = 24)
...
Two Gaussian clusters different mean, same variance

```{r 2-clust-dmsv, message = FALSE}
nCl <- 2
clust <- rep(seq_len(nCl), c(32, 32))
nPts <- length(clust)
points <- matrix(rnorm(nPts * hiDim, 1), nPts, hiDim) + (2 * clust)
distances <- dist(points)
cmds <- cmdscale(distances)
...
Two Gaussian clusters different mean and variance

```{r 2-clust-dmdv, message = FALSE}
nCl <- 2
clust <- rep(seq_len(nCl), c(32, 32))
nPts <- length(clust)
points <- (matrix(rnorm(nPts * hiDim), nPts, hiDim) * clust) + (2 * clust)
distances <- dist(points)
cmds <- cmdscale(distances)
...
```{r 3-clust, message = FALSE}
clN <- c(32, 32, 32)
nCl <- length(clN)
clust <- rep(seq_len(nCl), clN)
clVar <- rep(c(1, 1, 2), clN)
clOffset <- rep(c(-1, 0, 2), clN)

nPts <- length(clust)
points <- (matrix(rnorm(nPts * hiDim), nPts, hiDim) * clVar) + clOffset
distances <- dist(points)
cmds <- cmdscale(distances)

silmds <- ClusterMDS(clust, distances, cmds, trace = TRUE, stability = 24)

layout(LayoutRow(nCl, 1))
par(mar = rep(1.5, 4))
...
```{r simple-clusters, message = FALSE, eval = FALSE}
library("TreeDist")
par(mfrow = 1:2)
cl <- rep(1:4, 6:9)
nPts <- length(cl)
...
```{r where-to-start, message = FALSE, eval = FALSE}
