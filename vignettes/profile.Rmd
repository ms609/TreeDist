...
\code{ratchHits} and \code{ratchIter}, and small values of \code{searchHits} and
\code{searchiter}, so that many runs don't quite hit the optimal tree.
In a serious study, you would want to sample many more than 25 Ratchet hits (\code{ratchHits}).

```{R Suboptimal sampling}
suboptimals <- Ratchet(better.tree, my.prepdata, swappers=list(RootedTBRSwap),
                       returnAll=TRUE, suboptimal=1.5, ratchHits=25, ratchIter=5000, 
                       bootstrapHits=15, bootstrapIter=450,
                       searchHits=10, searchIter=50, rearrangements='TBR')
```
...
inference.} Systematic Biology 50:3, 331-350, doi:[10.1080/10635150118627](http://dx.doi.org/10.1080/10635150118627)
