...
```{r, fig.width=6 , fig.align='center'}
x <- seq(1, 16, by=0.1)
plot(0, xlim=range(x), ylim=c(3, 10),
     xlab = 'k', ylab = 'Jaccard-Robinson-Foulds distance', pch = 3, type='n')
lines(x, vapply(x, JaccardRobinsonFoulds, tree1=tree1, tree2=tree2, arboreal=FALSE, 0),
      col=cbPalette8[5])
lines(x, vapply(x, JaccardRobinsonFoulds, tree1=tree1, tree2=tree2, 0),
      col=cbPalette8[4])
rf <- RobinsonFoulds(tree1, tree2)
nts <- NyeTreeSimilarity(tree1, tree2, similarity = FALSE)
abline(h = rf, lty = 'dashed', col=cbPalette8[6])
abline(h = nts, lty = 'dotted', col=cbPalette8[8])
