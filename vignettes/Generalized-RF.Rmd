...
### The problem with prohibiting conflicting pairings

<!--TODO expand to expound arboreal matchings-->
Shared phylogenetic information prohibits the pairing of contradictory splits: 
that is, it assigns zero similarity to splits that cannot both occur on a 
single tree.  
...
_J_, yet receive zero similarity if conflicting splits cannot be paired.
...
<!--TODO REWRITE; coherent matching -> prohibit conflicting pairings -->
...
[@SmithDist]  (see [separate vignette](information.html)) results in a
 information-based tree distance metric that, as desired, assigns a
...
<!--TODO REWRITE -- I'd misunderstood what an arboreal matching was -->
...
Such a 'coherent' matching would preclude the pairing of `ABCFGHI|DE` with
`ABCDFGH|EI`:
