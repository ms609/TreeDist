...
```{R Cherry-short-move, include=FALSE, echo=FALSE, message=FALSE, cache=TRUE, fig.width=6.4, fig.height=4, out.width='75%', fig.align='center'}
...
```{r echo=FALSE, include=FALSE, results="asis", eval=FALSE}
tRow <- SetRow(c(T,T,T,T,T,NA,T,F,F), "Single-node move")
PrintLastSet()
```
--><!--
```{R Cherry-short-move-2, include=FALSE, echo=FALSE, message=FALSE, cache=TRUE, fig.width=6.4, fig.height=4, out.width='75%', fig.align='center'}
par(mfrow=c(1, 3), mar=standardMargin, cex=1)
...
<!--
```{R  Move-a-cherry-two-b, include=FALSE, echo=FALSE, message=FALSE, cache=TRUE, fig.width=6, fig.height=4, out.width='66%', fig.align='center', include=FALSE, eval=FALSE}
...
layout(matrix(c(rep(1:3, each=8), 3 + rep(1:8, each=3)), ncol=2), widths=c(0.33, 0.67))
...
```{R Tree-distances, echo=FALSE, include=FALSE, fig.align='center', fig.height=8, fig.width=6.4, message=FALSE, cache=TRUE, out.width='66%'}
...
```{R tree-spaces, echo=FALSE, eval=FALSE, include=FALSE, fig.align='center', fig.height=8, fig.width=6.4, message=FALSE, cache=TRUE, out.width='66%'}
...
TableImage <- function (method) {
  ...
  rasterImage(hShapeKey, 0, 955, 945, 975)
  rasterImage(vShapeKey, 955, 945, 975, 0)
...
Hist <- function (method, breaks=histBreaks[[method]]) {
  ...
  plot(h, col=MethodPalette(method, length(h$breaks) - 1L),
       main=abbrevs[method], axes=FALSE)
...
[@Von_luxburg2007]--> heirarchical clustering with average linkage,
as recommended for phylogenetic trees [@Lin2012;@Stockham2002];
...
PlotMDS <- function(method, metric=TRUE, showClusters=FALSE) {
  if (metric) {
  ...
  if (showClusters) {
    plot(pts, pch=19, xlab='', ylab='', axes=FALSE, col=clusters[, method])
  } else {
...
HClusters <- function (dat, method) {
  ...
  cutree(clusters, k = 8L)
}
clusters <- vapply(td7[methods], HClusters, method='average', integer(945L))
# Alternative using Spectal clustering:
