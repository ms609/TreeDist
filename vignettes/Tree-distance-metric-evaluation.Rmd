...
ColPlot <- function (tr, title=NULL, bold=NULL, ...) {
  ...
  plot(tr, tip.col=Ternary::cbPalette15[-c(4, 7)][as.integer(tr$tip.label)], 
       ...
       y.lim=c(-0.4 - length(legendSequence), 11), ...)
}
...
tRow <- matrix(nrow=10, ncol=9, dimnames=list(1:10, c('mai','mpi','clu','nye','msd','qd','path','spr','rf')))
attr(tRow, 'rowI') <- 0
...
Following on from this, exchanging two tips ought to result in a smaller distance
than the distance expected for a pair of random trees:

## Random trees and their distances
...
```{R Random-trees, echo=FALSE, cache=TRUE, warn=FALSE, message=FALSE, fig.asp=1.2, fig.width=5.6, out.width='80%', fig.align='center'}
par(mfrow=c(2, 8), mar=c(1.9, 0.2, 0.2, 0.2))
layout(matrix(c(1:8, rep(9, 8)), ncol=2), widths=c(0.8, 0.2))
...
```{r echo=FALSE, results="asis"}
tRow <- SetRow(c(mai=F,mpi=T,clu=T,nye=T,msd=T,qd=T,path=F,spr=T,rf=F), 'Exchange two tips')
PrintLastSet()
```
...
```{r echo=FALSE, results="asis"}
tRow <- SetRow(c(mai=T,mpi=T,clu=T,nye=T,msd=T,qd=T,path=F,spr=T,rf=F), 'Complexity')
PrintLastSet()
```
...
```{r echo=FALSE, results="asis"}
tRow <- SetRow(c(mai=T,mpi=T,clu=T,nye=T,msd=T,qd=T,path=T,spr=NA,rf=F), 'Saturation')
PrintLastSet()
```
...
```{r echo=FALSE, results="asis"}
tRow <- SetRow(c(mai=T,mpi=T,clu=T,nye=T,msd=T,qd=F,path=F,spr=T,rf=T), 'Islands')
PrintLastSet()
```
-->
...
```{r echo=FALSE, results="asis"}
tRow <- SetRow(c(mai=T,mpi=T,clu=T,nye=F,msd=F,qd=T,path=F,spr=F,rf=F), 'Meaningful units')
PrintLastSet()
```
...
The figure below shows how the normalized tree distance varies with the number of 
...
(downward triangle`r ''#&#9661;`) are normalized based on the maximum possible value.
...
```{r echo=FALSE, results="asis"}
tRow <- SetRow(c(mai=T,mpi=T,clu=T,nye=T,msd=F,qd=T,path=F,spr=NA,rf=T), 'Value has context')
PrintLastSet()
```
...
```{r echo=FALSE, results="asis"}
tRow <- SetRow(c(mai=T,mpi=T,clu=T,nye=T,msd=T,qd=T,path=F,spr=F,rf=T), 'Polytomies')
PrintLastSet()
```
