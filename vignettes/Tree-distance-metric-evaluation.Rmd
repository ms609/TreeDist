...
ClusterBars <- function(method, clusterMethod = 'pam', nClusters = 8L) {
  ret <- double(length(treeCols))
  names(ret) <- names(treeCols)
  
  clusters <- 
  if (clusterMethod == 'pam') {
    cluster::pam(td7[[method]], nClusters, diss=TRUE, cluster.only=TRUE)
  } else if(clusterMethod == 'spectral') {
    SpectralClustering(maxDist[method] - td7[[method]], nClusters)
  }
...
lapply(plotSequence, ClusterBars, clusterMethod='spectral', nClusters = 8L) -> XX
