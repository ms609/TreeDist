...
```{r}
NyeTreeSimilarity(tree1, tree2, normalize = min)
```
...
```{r}
NyeTreeSimilarity(list(tree1, tree2), list(tree1, tree2), normalize = pmin)
```
...
```{r}
NyeTreeSimilarity(tree1, tree2, normalize = TRUE)
```
...
if (requireNamespace('TreeDistData', quietly = TRUE)) {
  ...
  methodCol <- c(dpi = '#e15659', cid = '#58a14e', nts = '#edc949',
                 qd = '#af7aa1')
  
  par(cex = 0.7, mar = c(5, 5, 0.01, 0.01))
  nLeaves <- as.integer(dimnames(randomTreeDistances)[[3]])
  plot(nLeaves, type = 'n', randomTreeDistances['dpi', 'mean', ],
       ylim = c(0.4, 1),
