...
leaves in the trees being compared.
...
The default normalization behaviour of each functions in TreeDist is listed
...
[`NSplits`](https://ms609.github.io/TreeTools/reference/NSplits.html)`(11)`
...
to two identical _fully bifurcating_ eleven-leaf trees; this is the default
...
maximum possible distance.  The diameter of other metrics is more difficult
to calculate.  
...
present in a pair of trees.  But with even a modest number of leaves, no pairs
of trees exist in which every split in one tree is perfectly contradicted
by every other split in the other; as such, any pair of trees will 
...
plot(nLeaves, type='n', randomTreeDistances['vpi', 'mean', ], ylim=c(0.4, 1), 
     xlab = "Number of leaves", ylab = "Normalized distance between random trees")
...
ClusteringInfoDistance(tree1, tree2, normalize = TRUE) / expectedVCI
```
...
because it contains 80% of the splits in the target tree, and no 
incorrect splits -- or because it is perfectly bifurcating, but 
...
contain more information than two very uneven split, so the absence of 
...
resolution by taking a consensus with three trees that are two NNI moves away.
...
We'll define some colours first: 
<span style='background-color:#F5CCB599'>&nbsp;orange&nbsp;</span> 
for the one-away trees, and 
