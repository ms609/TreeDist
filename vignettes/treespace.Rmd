...
may be easier to interpret than the clusters' hypervolume.
...
or the average distance from the centroid:
```{r ave-dist-centroid}

```

The density of points within a cluster can be measured using the average nearest-neighbour distance:
```{r ave-nnd}

```

or the average minimum spanning tree edge length:
```{r ave-mst-edge}
