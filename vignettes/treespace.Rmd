...
First we'll select some trees to analyse.  
In this simple example we'll take a series of 218 trees from a list ordered by
the trees' mixed-base representation.
...
the clustering information distance is both reliable and quick to calculate
[@SmithDist].
...
Other distances are available, and give results that differ in instructive ways:
...
```{r plot-projection-2d}
plot(projection, 
     asp = 1, # Preserve aspect ratio - do not distort distances
     ann = FALSE, axes = FALSE, frame.plot = TRUE, # Simple border
     col = treeCols, pch = 16
     )
```
...
\> 0.5 denotes good evidence of clustering, and \> 0.7 strong evidence [@ref].
...
One thing we may wish to do is to take the consensus of each cluster:
...
So it looks like our clusters differ in the position of `t3`.
...
```{r calculate-MST}
umst <- suppressWarnings(ape::mst(distances))
edges <- umst == 1L
from <- umst
from[edges] <- unlist(apply(edges, 1, which))
to <- t(from)
mstEnds <- cbind(from[lower.tri(from) & edges],
                 to[lower.tri(to) & edges])
