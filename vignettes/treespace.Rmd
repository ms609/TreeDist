...
or the mean distance from the centroid:
```{r ave-dist-centroid}
MeanCentroidDistance(pid_mapping, pid_cluster)
```

The density of points within a cluster can be measured using the mean nearest-neighbour distance:
```{r ave-nnd}

```

or the mean minimum spanning tree edge length:
```{r ave-mst-edge}
