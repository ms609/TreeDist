...
```{r Consensus}
some.optimal.trees <- MultiRatchet(best.tree, my.phyDat, swappers=
...
nodelabels(paste('\n\n', node.frequency), col=SupportColour(node.frequency), 
...
jack.trees <- Jackknife(best.tree, my.phyDat, EdgeSwapper=RootedTBRSwap,
                        jackIter=10, verbosity=1)
jack.support <- SplitFrequency(strict.consensus, jack.trees)
par(mar=rep(0.25, 4), cex=0.75) # make plot easier to read
plot(strict.consensus)
```
