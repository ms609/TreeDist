...
Using `RootedNNICore` instead of `NNICore` stops taxa from moving from one side of the root to another,
which we need to do if we're going to enforce our outgroup constraint.  Even if we are using a
single-taxon outgroup, using `RootedNNICore` retains the position of the root, which makes tree
comparison and interpretation easier later on.

```{r Basic NNI search}
better.tree <- MorphySearch(tree=rooted.tree, dataset=my.phyDat, 
                           Rearrange=RootedNNICore, verbosity=3)
```
...
```{r SPR and TBR}
better.tree <- MorphySearch(better.tree, my.phyDat, maxHits=8, maxIter=10000,
                           Rearrange=RootedSPRCore, verbosity=2)
better.tree <- MorphySearch(better.tree, my.phyDat, maxHits=20, maxIter=40000, 
                           Rearrange=RootedTBRCore, verbosity=2)
```
...
```{r Ratchet search}
best.tree <- MorphyRatchet(better.tree, my.phyDat, verbosity=0, k=5,
...
my.consensus <- RatchetConsensus(best.tree, my.phyDat, rearrangements=
                                 list(RootedTBRCore, RootedNNICore))
```
