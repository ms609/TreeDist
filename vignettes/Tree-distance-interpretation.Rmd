...
if (requireNamespace('TreeDistData', quietly = TRUE)) {
  ...
  totalInfoContent <- 3L
  methodLCH <- colorspace::coords(as(colorspace::hex2RGB(TreeDistCol(methods)),
                                     'polarLUV'))
  ...
  oldPar <- par(mfrow = c(4, 1), cex = 0.7, oma = rep(0, 4),
                mar = c(5, 5, 0.01, 0.01))
...
normalized against $n$. The upper and lower bounds on the NNI distance are
normalized against the upper and lower bounds of the diameter of the NNI
distance for trees of the corresponding sizes [@Li1996;@Fack2002].
...
approximate maximum, estimated by a log-linear model
