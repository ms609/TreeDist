...
The Robinson-Foulds distance is a symmetric distance measure that counts the 
...
clades, edges or nodes.  If a tree is drawn as rooted (even without a root edge),
then the number of partitions is one less than the number of edges and two less
than the number of nodes. For example, the trees below have one partition, 
two edges and three nodes in common.

```{R, echo=FALSE, fig.width = 6, fig.align = 'centre'}
TwoTreePlot()
TreeDistPlot(balancedTree <- 
               read.tree(text="(((1, 2), (3, 4)), ((5, 6), (7, 8)));"),
             leaveRoom = FALSE)
edgelabels(1:6, edge = c(1, 2, 5, 8, 9, 12), 
           adj = c(-0.5, 0.5), bg = cbPalette8[8])
nodelabels(1:7, bg = cbPalette8[3], adj=c(1, 0.5))
nodelabels(1:6, node = 10:15, bg = cbPalette8[2], adj=c(-1, 0.5))
edgelabels(1:5, c(1, 2, 5, 9, 12), adj = c(1.5, 0.5), bg = cbPalette8[5])
legend('bottomleft', col=cbPalette8[c(5, 8, 3, 2)], pch=15, bty='n',
       c('Partitions (5)', 'Internal edges (6)', 'Nodes (7)', 'Clades (6)'))

TreeDistPlot(read.tree(text="((1, 2, 3, 4), (5, 6, 7, 8));"), leaveRoom = FALSE)
edgelabels(1:2, edge = c(1, 6), adj = c(-0.5, 0.5), bg = cbPalette8[8])
nodelabels(1:2, node=10:11, bg = cbPalette8[2], adj = c(-1, 0.5))
nodelabels(1:3, bg = cbPalette8[3], adj = c(1, 0.5))
edgelabels(1, 1, adj = c(1.5, 0.5), bg = cbPalette8[5])
...
A partition distance of 1 could mean that two trees differ in an less
informative partition, or a more informative partition.  In the example above, 
...
In contrast, a tree with a clade of two taxa and one of six has a one in seven 
...
even partition is less noteworthy than failure to match an even one, even though
...
ape::edgelabels(1:5, c(1, 2, 5, 9, 12), bg = cbPalette8[5], adj=c(0.5, -0.25))
...
ape::edgelabels(1:5, bg = cbPalette8[5], c(4, 6, 8, 10, 12), adj=c(0.5, -0.25))
...
par(cex=0.8)
sch <- hist(inCaterpillar + 0.5, breaks = 0:12 / 2 - 0.0001,
...
The Robinson-Foulds distance also suffers from easy saturation: the maximum
...
This feels unsatisfactory: if it wasn't for that single taxon _I_, the trees 
differ only in the resolution of a single node:

```{r}
VisualizeMatching(RobinsonFouldsInfo, drop.tip(tree1, 'I'), drop.tip(tree2, 'I'))
```

The solution is to assign non-zero similarity scores to partitions that are
similar, but not identical: here, each pair of partitions is scored according
to its mutual phylogenetic information.

```{r}
VisualizeMatching(MutualPhylogeneticInfo, tree1, tree2)
```

Here, the partitions HI:ABCDEFG and EI:ABCDFGH have no information
in common; they may as well not have been paired, and they contribute nothing
to the tree similarity score.  The partition AB:CDEFGHI occurs in both trees,
and, as it happens, makes the largest contribution to the tree similarity 
score for this particular pair of trees. 
