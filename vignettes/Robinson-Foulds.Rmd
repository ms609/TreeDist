...
In the first tree, split 1 is even, dividing four taxa from four others (`4|4`); splits 2--5 are maximally uneven (`2|6`).  The total information content of these five splits is `r signif(sum(ic[c(4, 2, 2, 2, 2) - 1]), 4)` bits, whereas that of the five splits in 
the second tree, of sizes `2|6`, `3|5`, `4|4`, `3|5` and `2|6`, is
`r signif(sum(ic[c(2, 3, 4, 3, 2) - 1]), 4)` bits. Put another way, a random tree will
...
whereas the second pair differ in the resolution of a more even split
...
scored according to the similarity of the paired splits.  The decision
of which split to pair with which is called a _matching_, and a matching
...
Let's consider two trees that differ in the position of a wildcard leaf,
...
However, the Robinson-Foulds distance will see only one split, `AB|CDEFGHI`,
...
the Robinson-Foulds distance is the number of splits present in one tree 
but not the other.

The solution is to assign non-zero similarity scores to splits that are
similar, but not identical.

We discussed above weighting each matching pair of splits according to the
amount of phylogenetic information represented by the split.
A non-matching pair of splits can be scored according to the 
shared phylogenetic information, which is a function of the proportion of trees
...
exist on a single tree, such as `ABCDEFG|HI` - `ABCDFGH|EI` -- have no
phylogenetic information in common.
...
DisplaySplits <- function (tree) 
  as.list(as.character(TreeTools::as.Splits(tree)))
DisplaySplits(tree1)
```
```{r}
DisplaySplits(tree2)
...
alternative matching, still optimal, would leave these splits unpaired.

## Alternative approaches

Alternatives measures of split similarity are implemented in other 
