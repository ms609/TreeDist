...
If all outcomes are equally likely, then the information content of
...
- Information content: $H(A) = -\log_2(\frac{1}{2})$ = 1 bit.

B: "Is the roll greater than 3?"

- Two equally-possible outcomes: yes or no
- Information content: $H(B) = -\log_2(\frac{1}{2})$ = 1 bit.
...
The variation of information, also termed the entropy distance [@Mackay2003, 140],
is the information that _A_ and _B_ do _not_ have in common:

\begin{aligned}
VoI(A, B) = H(A, B) - I(A;B) = 2H(A,B) - H(A) - H(B)
\end{aligned}
...
When it comes to comparing splits [@SmithDist], we can imagine drawing at
random one binary _n_-leaf tree.

Phylogenetic information is related to the probability of drawing a given tree
...
$P(x) = \frac{1}{105}$.
The Shannon information content of this outcome is $h(x) = -\log_2{P(x)}$.
Because all outcomes are equiprobable, the entropy of the ensemble _X_ $H(X) =
...
```{r, fig.width=7, out.width='90%', fig.height=7*3/5, echo=FALSE, message=FALSE}
library('TreeTools') # For Splits
par(mfrow=c(3, 5), mar=rep(0.4, 4), cex=0.9)
ShowNode <- function (match, label, col) {
  if (any(!is.na(match)))
    ape::nodelabels(label, bg = col, height=1.3,
                    node = as.integer(names(splits)[match]))
}
trees <- TreeTools:::as.phylo.numeric(0:14, tipLabels = 2:6)
trees <- structure(lapply(trees, AddTip, 1, 1), class='multiPhylo')
A <- as.Splits(c(FALSE, TRUE)[c(1, 1, 1, 2, 2, 2)], tipLabels=1:6)
B <- as.Splits(c(FALSE, TRUE)[c(1, 1, 1, 1, 2, 2)], tipLabels=1:6)
for (tree in trees) {
  splits <- as.Splits(tree, tipLabels = 1:6)
  matchA <- match.Splits(A, splits)
  matchB <- match.Splits(B, splits)
  TreeDistPlot(tree, leaveRoom = FALSE)
  ShowNode(matchA, ' ABC | DEF ', col = Ternary::cbPalette15[5])
}
```

`BC|ADE` occurs in three of the fifteen 5-leaf trees; `DE|ABC` occurs in 
three trees; and just one tree contains both splits.

The probability of drawing any given tree _x_, given that we may only select
5-leaf trees containing `BC|ADE`, is thus $P(x|\textrm{(BC|ADE)}) = \frac{1}{3}$.
Likewise,  $P(x|\textrm{(DE|ABC)}) = \frac{1}{3}$.

The entropy of the ensemble implied by the split `BC|ADE`
is $H(BC|ADE) = -log_2{frac{1}{3}}$.

The joint entropy of the splits `BC|ADE` and `DE|ABC`, 
_H_(`BC|ADE`, `DE|ABC`), is zero: there is but one tree consistent with both
splits, so $P(x) = 1$.

If two splits $S_1, S_2$ are compatible with $n_1, n_2$ equiprobable trees, 
then the relative entropy (Kullback-Leibler divergence) is given by
$D_KL (S_1||S_2) = \log_2{\frac{n_2}{n_1}}$




<!--
The Shannon information content of each split [@Mackay2003, 32;
the phylogenetic information content, _sensu_ @Steel2006] is:

- _h_(`BC|ADE`) = $-\log_2(\frac{3}{15})$
- _h_(`DE|ABC`) = $-\log_2(\frac{3}{15})$
-->
The entropy of an ensemble _X_ is the average information content of an outcome:

- _H_(`BC:ADE`) = $-\frac{3}{15}\log_2(\frac{3}{15})$
- _H_(`DE:ABC`) = $-\frac{3}{15}\log_2(\frac{3}{15})$
