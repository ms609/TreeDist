...
In the case _n_ = 5, taking all fifteen binary 5-leaf trees to be equally 
probable, the probability of drawing any particular tree is 
$P(x) = \frac{1}{15}$.  The Shannon information content of tree _x_ is 
$h(x) = -\log_2{P(x)}$, and the entropy of the ensemble _X_ of all possible
trees is $H(X) = \sum\nolimits_{x\in n\textrm{-leaf trees}}P(x)\log_2{\frac{1}{P(x)}} = -\log_2{P(x)}$

Let's consider the two splits `BC|ADE` and `DE|ABC` in the universe of 5-leaf
trees.
