...
grf_match nni_rf_matching (
    ...
    if (n_tips > NNI_MAX_TIPS) {
      Rf_error("Cannot calculate NNI distance for trees with so many tips.");
    }
    
    // #nocov start
    if (static_cast<int64_t>(n_splits) * static_cast<int64_t>(n_bins) >
          static_cast<int64_t>(std::numeric_limits<int32_t>::max())) {
      Rf_error("Cannot calculate NNI distance for trees with so many splits.");
    }
    // #nocov end
...
IntegerVector cpp_nni_distance(const IntegerMatrix& edge1, 
  ...
  if (n_edge != int32_t(edge2.nrow())) {
    Rf_error("Both trees must have the same number of edges. "
  ...
  if (n_splits < 1) {
    Rf_error("NNI distance is undefined for trees with no splits"); // #nocov
  }
