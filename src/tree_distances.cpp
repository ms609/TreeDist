...
namespace TreeDist {
  ...
  inline void add_ic_element(double& ic_sum, int32 nkK, int32 nk, int32 nK,
                             int32 n_tips) noexcept {
    /* 
  ...
  template<typename T, size_t StackSplits, size_t StackBins>
  class SplitScratch {
  public:
    SplitScratch(size_t splits, size_t bins)
      : splits_(splits), bins_(bins),
        use_heap_(splits > StackSplits || bins > StackBins)
    {
      if (use_heap_) {
        heap_.reset(new T[splits * bins]);
        data_ = heap_.get();
      } else {
        data_ = &stack_[0][0];
      }
    ...
    inline T* operator[](size_t i) {
      return data_ + i * bins_;
    }
    
  private:
    size_t splits_, bins_;
    bool use_heap_;
    std::unique_ptr<T[]> heap_;
    T* data_;
    T stack_[StackSplits][StackBins];
  };
  
}

using TreeDist::resize_uninitialized;
using TreeDist::SplitScratch;
...
inline List robinson_foulds_distance(const RawMatrix &x, const RawMatrix &y,
  ...
  SplitScratch<splitbit, SL_MAX_SPLITS, SL_MAX_BINS>
    b_complement(b.n_splits, a.n_bins);
...
inline List matching_split_distance(const RawMatrix &x, const RawMatrix &y,
  ...
  const int32 split_diff = most_splits - std::min(a.n_splits, b.n_splits);
  const int32 half_tips = n_tips / 2;
  if (most_splits == 0) {
  ...
  // More performant to cleave this out, to keep the previous loop simpler
  for (int32 ai = 0; ai < a.n_splits; ++ai) {
    for (int32 bi = 0; bi < b.n_splits; ++bi) {
...
List msi_distance(const RawMatrix &x, const RawMatrix &y, const int32 n_tips) {
  ...
  const double max_possible = lg2_unrooted[n_tips] - 
    lg2_rooted[int32((n_tips + 1) / 2)] - lg2_rooted[int32(n_tips / 2)];
  ...
  for (int32 ai = 0; ai < a.n_splits; ++ai) {
    for (int32 bi = 0; bi < b.n_splits; ++bi) {
      ...
      const int32 n_same = n_tips - n_different;
...
List mutual_clustering(const RawMatrix &x, const RawMatrix &y,
  ...
  const int32 most_splits = a_has_more_splits ? a.n_splits : b.n_splits;
  const int32 a_extra_splits = a_has_more_splits ? most_splits - b.n_splits : 0;
  const int32 b_extra_splits = a_has_more_splits ? 0 : most_splits - a.n_splits;
  ...
  std::unique_ptr<int32[]> b_match = std::make_unique<int32[]>(b.n_splits);
  ...
  const int32 lap_dim = most_splits - exact_matches;
  ASSERT(lap_dim > 0);
  std::vector<lap_col> rowsol;
  std::vector<lap_row> colsol;
  ...
  if (exact_matches) {
    ...
    std::unique_ptr<int32[]> lap_decode = std::make_unique<int32[]>(lap_dim);
    int32 fuzzy_match = 0;
    for (int32 bi = 0; bi < b.n_splits; ++bi) {
      if (!b_match[bi]) {
    ...
    for (int32 i = 0; i < a.n_splits; ++i) {
      if (a_match[i]) {
        ...
        assert(fuzzy_match < lap_dim);
        const int32 row_idx = fuzzy_match++;
        const int32 col_idx = rowsol[row_idx];
